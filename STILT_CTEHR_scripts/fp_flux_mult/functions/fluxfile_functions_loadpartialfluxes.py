...
def open_fluxfiles(fluxfile_list, sim_len=240, mp_start_date=None, mp_end_date=None, fluxtype=None, variablelist_vars=None):
    ...
    # Add time variable to variablelist_vars to put the values in the dictionairy later
    variablelist_vars = ['time'] + variablelist_vars
    
    if fluxtype == "PARIS":        
        with xr.open_dataset(fluxfile_list) as ds:
            ...
            # Fill dictionairy with all variables in variablelist_vars
            for var in variablelist_vars:
                fluxdict[var] = ds[var][:].compute().values

            #return subds[variablelist_vars].to_dict(data='array')

...
sparse_files = sorted(glob.glob(filepath+'footprint_' + station + '*.csv'))
dense_files = sorted(glob.glob(filepath+'footprint_' + station + '*.nc'))

#sparse_files = [filepath + 'footprint_CES_2021x07x05x16x51.97Nx004.93Ex00200.csv']
...
for i in range(0,len(sparse_files)):
    ...
    timelist = fp_df_sparse['Time'].unique()
    ...
    for y in range(0,len(timelist)):
        ...
        fp_curdf_sparse = fp_df_sparse[fp_df_sparse['Time'] == timelist[y]]

        # Extract latitude indices from sparse footprint file using list comprehension
        lat_indices = [lats.index(i) for i in fp_curdf_sparse['Latitude'].to_list()]
        lon_indices = [lons.index(i) for i in fp_curdf_sparse['Longitude'].to_list()]

        # Fill flux_Influence_list with values from CTE-HR flux file
        flux_Influence_list_sparse += (cte_ds.values[hours_into_file,
                                            lon_indices, 
                                            lat_indices] * fp_curdf_sparse['Influence']).tolist()
        
        flux_Influence_list_dense += (cte_ds.values[hours_into_file,
                                            lon_indices,
                                            lat_indices] * fp_df_dense.variables['Influence'][lon_indices,
                                            lat_indices,
                                            timelist[y]]).tolist()
    
    # Add list to fp df
    fp_df_sparse['mixed_sparse'] = flux_Influence_list_sparse
    fp_df_sparse['mixed_dense'] = flux_Influence_list_dense
    fp_df_sparse['dif'] = fp_df_sparse['mixed_sparse'] - fp_df_sparse['mixed_dense']
    
    # Create output file name
    outfile = outpath + os.path.basename(sparse_file)[:-4] + '_molefrac_sparse_vs_dense.csv'

    # Save fp df to csv
    fp_df_sparse.to_csv(outfile, index=False)

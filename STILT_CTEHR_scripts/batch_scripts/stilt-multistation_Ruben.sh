...
then
  ...
  path="/projects/0/ctdas/PARIS/DATA/footprints/wur/${altdir}/"
fi
...
do
  ...
  Rscript stilt_loop_dense_and_sparse.r --sparse --overwrite-localization --station $station --stationfile $FILENAME --nhrs 240 --npars 250 --path $path --rundir $rundir --sourcepath $sourcepath >> "${subdir}STILT_log" &  # Run your executable, note the "&"

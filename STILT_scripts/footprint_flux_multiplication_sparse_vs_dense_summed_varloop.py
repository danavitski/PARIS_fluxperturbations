...
for i in range(0,len(sparse_files)):
    ...
    lat_indices = [lats.index(i) for i in np.round(fp_df_sparse['Latitude'], 4).to_list()]
    lon_indices = [lons.index(i) for i in np.round(fp_df_sparse['Longitude'], 4).to_list()]

    # Select current footprint time 
    hours_into_file = hours_since_start_of_ds - fp_df_sparse['Time'].values

    ObsPack.variables['pseudo_observation'][i] = sum([(cte_ds.variables[var].values[hours_into_file,
                                                    lat_indices,
                                                    lon_indices] * fp_df_sparse['Influence']).sum() for var in fluxnamelist])
